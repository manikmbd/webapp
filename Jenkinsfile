pipeline {
  ...
  stages {
    ...
    stage ('Deploy-To-Tomcat') {
            steps {
           sshagent(['tomcat']) {
                sh 'scp -o StrictHostKeyChecking=no target/*.war ubuntu@3.111.36.214:/var/lib/tomcat9/webapps/webapp.war'
              }      
           }       
    }
    ...
    stage ('Port Scan') {
		    steps {
			sh 'rm nmap* || true'
			sh 'docker run --rm -v "$(pwd)":/data uzyexe/nmap -sS -sV -oX nmap 3.111.36.214'
			sh 'cat nmap'
		    }
	    }
    ...
    stage ('Nikto Scan') {
			...
			sh 'docker run --user $(id -u):$(id -g) --rm -v $(pwd):/report -i secfigo/nikto:latest -h 3.111.36.214 -p 8080 -output /report/nikto-output.xml'
    ...
    stage ('Nuclei') {
		...
		sh 'docker run -t projectdiscovery/nuclei -u http://3.111.36.214:8080/webapp/ --json > nuclei'
		sh 'cat nuclei'
	    }
	    }
        }
